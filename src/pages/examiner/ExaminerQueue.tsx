...
const ExaminerQueue = () => {
  ...
  const submit = (projectId: string) => {
    eventBus.publish({ type: "audit.note", actorId: "ext-001", message: `External examiner verdict submitted for ${projectId}` });
  ...
  return (
    <div className="animate-fade-in">
      <PageHeading
        title="External Examination Queue"
        subtitle="Blind-review submissions assigned to you"
        badge={<Badge variant="outline" className="text-primary border-primary/30 gap-1"><Gavel className="h-3 w-3" /> External Examiner</Badge>}
      />
          ...
          {queue.map((p) => (
            <div key={p.id} className="flex items-center justify-between p-3 rounded-lg border border-border/60">
              <div className="min-w-0">
                <p className="font-medium text-sm truncate">[Anonymized] {p.title}</p>
