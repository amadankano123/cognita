...
const Analysis = () => {
      ...
      setProgress((prev) => {
        if (prev >= 100) {
          ...
          setResult({
            rSquared: 0.82,
            pValue: 0.002,
            interpretation: "The regression model shows a statistically significant relationship between model prediction and confidence score (p < 0.05). The model explains 82% of the variance in confidence scores, indicating a strong predictive relationship.",
          });
          return 100;
        }
  ...
  const handleSaveToResults = () => {
    addAnalysisResult({
      id: `a${Date.now()}`,
      title: `${analysisType}: ${dependent} ~ ${independent}`,
      type: analysisType,
      status: "completed",
      summary: `R² = ${result!.rSquared}, p-value = ${result!.pValue}. ${result!.interpretation}`,
        ...
        <Card className="shadow-card p-5 lg:col-span-1">
          ...
          <div className="space-y-4">
              ...
              <>
                <div>
                  ...
                  <Select value={dependent} onValueChange={setDependent}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {project.dataset.columns.filter((c) => c.type === "number").map((c) => (
                ...
                <div>
                  ...
                  <Select value={independent} onValueChange={setIndependent}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {project.dataset.columns.filter((c) => c.name !== dependent).map((c) => (
                        <SelectItem key={c.name} value={c.name}>{c.name}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
            ...
            <Button className="w-full" onClick={handleRun} disabled={running}>
              <Play className="h-4 w-4 mr-2" /> {running ? "Running…" : `Run ${analysisType}`}
            </Button>
        ...
        <div className="lg:col-span-2 space-y-4">
          {result && (
            <Card className="shadow-card p-5 animate-fade-in">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-display font-semibold">Regression Results</h3>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-muted/50 rounded-lg p-4 text-center">
                  <p className="text-3xl font-bold text-primary">{result.rSquared}</p>
                  <p className="text-xs text-muted-foreground mt-1">R² (Coefficient of Determination)</p>
                </div>
                <div className="bg-muted/50 rounded-lg p-4 text-center">
                  <p className="text-3xl font-bold text-primary">{result.pValue}</p>
                  <p className="text-xs text-muted-foreground mt-1">p-value</p>
                </div>
              </div>

              <div className="bg-accent/30 rounded-lg p-4 mb-4">
                <h4 className="text-xs font-semibold text-muted-foreground uppercase mb-2">AI Interpretation</h4>
                <p className="text-sm leading-relaxed">{result.interpretation}</p>
              </div>

              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={handleSaveToResults}>
                  <Save className="h-4 w-4 mr-1" /> Save to Results Repository
                </Button>
                <Button variant="outline" size="sm" onClick={handleInsertIntoDoc}>
          ...
          <Card className="shadow-card">
            <div className="p-4 border-b border-border">
              <h3 className="font-display font-semibold">Previous Analyses</h3>
            </div>
