...
function simulateGenericAiContent(sectionTitle: string, projectTitle: string): string {
  const map: Record<string, string> = {
    ...
    "Literature Review": `A systematic review of the literature reveals three dominant themes in this field:\n\n**Theme 1: Foundational Approaches**\nEarly work established baseline methodologies that subsequent studies built upon.\n\n**Theme 2: Modern Computational Methods**\nRecent advances in machine learning and deep learning have transformed the field.\n\n**Theme 3: Deployment and Accessibility**\nA growing body of literature addresses the gap between research accuracy and practical deployment.`,
    "Methodology": `**Research Design**\nThis study employs a quantitative experimental research design with a comparative evaluation framework.\n\n**Data Collection**\nData was systematically collected from multiple sources.\n\n**Analysis Framework**\nThe analysis pipeline consists of three stages: (1) feature extraction, (2) model training with cross-validation, and (3) statistical evaluation.`,
    "Results": `The experimental results demonstrate the effectiveness of the proposed approach:\n\n**Primary Findings:**\n- The proposed method achieved superior performance compared to all baseline approaches (p < 0.001)\n- Robustness testing confirmed consistent performance across varied conditions\n\n**Statistical Significance:**\nAll primary comparisons reached statistical significance at α = 0.05.`,
    "Discussion": `The findings contribute to the field in several important ways:\n\n**Theoretical Implications:**\nThe results support and extend existing theoretical frameworks.\n\n**Practical Implications:**\nThe demonstrated feasibility of deployment on resource-constrained devices opens new avenues.\n\n**Limitations:**\nSeveral limitations should be acknowledged.`,
    "Conclusion": `This study has achieved its primary objectives. The key contributions include: (1) a novel methodology with demonstrated superiority over existing baselines, (2) comprehensive evaluation under both controlled and real-world conditions, and (3) practical deployment considerations.`,
  };
  return map[sectionTitle] || `AI-generated content suggestion for the "${sectionTitle}" section of your research on "${projectTitle}".`;
}
...
const Editor = () => {
  ...
  const handleGenerateSectionContent = () => {
    ...
    setTimeout(() => {
      ...
      if (activeSection) updateSection(activeSection.id, content);
          ...
          <TabsContent value="ai-assist" className="p-3 space-y-4 mt-0">
            <div className="space-y-2">
              ...
              <div className="flex gap-1.5">
                <Input value={aiTopicInput} onChange={e => setAiTopicInput(e.target.value)} placeholder="Enter or generate a topic…" className="text-xs h-8" />
                ...
                <Collapsible key={sub.id} open={expandedSubs.has(sub.id)} onOpenChange={() => toggleExpanded(sub.id)}>
                  <div className="border border-border rounded-lg overflow-hidden mb-1.5">
                    <div className="flex items-center justify-between px-3 py-2 bg-muted/30">
                      ...
                      <Button size="sm" variant="ghost" className="h-7 px-2 text-[10px] gap-1" onClick={(e) => { e.stopPropagation(); handleGenerateSubSection(sub.id); }} disabled={generatingId === sub.id}>
                    ...
                    <CollapsibleContent>
                      {sub.generated ? (
                        <div className="p-3 space-y-2">
                          <div className="text-xs text-foreground leading-relaxed whitespace-pre-line max-h-40 overflow-y-auto bg-background rounded p-2 border border-border">{sub.generated}</div>
                          <Button size="sm" className="h-7 text-[10px] w-full gap-1" onClick={() => handleInsertSubContent(sub.generated)}>
                            <Download className="h-3 w-3" /> Insert into Introduction
                          </Button>
                        </div>
                      ) : (
                        <div className="p-3 text-center"><p className="text-[10px] text-muted-foreground">Click "Generate" to create content</p></div>
                      )}
                    </CollapsibleContent>
                  </div>
                </Collapsible>
            ...
            <div className="border-t border-border pt-3 space-y-2">
              ...
              <div className="grid gap-1.5">
                ...
                <Button size="sm" variant="outline" className="justify-start h-8 text-xs gap-2" onClick={() => {
                  const s = project.sections.find(s => s.title === "Literature Review");
                  if (s) { setActiveSectionId(s.id); setTimeout(handleGenerateSectionContent, 100); }
                }}>
                  <Sparkles className="h-3 w-3 text-primary" /> Generate Literature Review
                </Button>
                <Button size="sm" variant="outline" className="justify-start h-8 text-xs gap-2" onClick={() => {
                  const s = project.sections.find(s => s.title === "Methodology");
                  if (s) { setActiveSectionId(s.id); setTimeout(handleGenerateSectionContent, 100); }
                }}>
      ...
      {activeSection && (
        <div className={cn(
          ...
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" className="gap-1.5 h-7 text-xs" onClick={handleGenerateSectionContent} disabled={aiGeneratingSection}>
              {aiGeneratingSection ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />}
              AI Generate
            </Button>
          </div>
        </div>
