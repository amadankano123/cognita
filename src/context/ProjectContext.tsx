...
export const ProjectProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const uploadDataset = useCallback(() => {
    setProject((prev) => ({
      ...prev,
      dataset: { ...prev.dataset, uploaded: true },
      checklist: prev.checklist.map((c) =>
        c.id === "cl5" ? { ...c, checked: true } : c
      ),
    }));
  }, []);
  ...
  const addExport = useCallback((record: ExportRecord) => {
    setProject((prev) => ({
      ...prev,
      exports: [...prev.exports, record],
      checklist: prev.checklist.map((c) =>
        c.id === "cl7" ? { ...c, checked: true } : c
      ),
    }));
  }, []);
  ...
  const insertCitation = useCallback((sectionId: string, citation: string) => {
    setProject((prev) => ({
      ...prev,
      sections: prev.sections.map((s) =>
        s.id === sectionId ? { ...s, content: s.content + " " + citation } : s
      ),
    }));
  }, []);
